...
func (ps P2PService) Assess(ctx *gin.Context) {
	...
	if err != nil {
		ps.sendError(ws, "不支持的接入类型")
		return
	}
	var msg Message
	if err := json.Unmarshal(message, &msg); err != nil {
		ps.sendError(ws, "不支持的消息类型")
		return
	}
	if msg.Event == "register" {
		// NSB注册
		id, _ := strconv.ParseInt(strings.Trim(string(msg.Data), `"`), 10, 64)
		info, err := ps.Data.GetDevice(id)
		if err != nil {
			ps.sendError(ws, "发生意料之外的错误")
			return
		} else if info == nil {
			ps.sendError(ws, "设备不存在")
			return
		} else {
...
// 转发消息
func (ps P2PService) relay(msg Message) {
